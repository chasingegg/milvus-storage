...
public:
    ...
    bool validateFile(const std::string& originalPath, const std::string& newPath) {
        std::ifstream f1(originalPath, std::ios::binary | std::ios::ate);
        std::ifstream f2(newPath, std::ios::binary | std::ios::ate);
        ...
        if (f1.tellg() != f2.tellg()) {
            std::cerr << "Validation failed: File sizes do not match. "
                      << originalPath << ": " << f1.tellg() << " bytes, "
                      << newPath << ": " << f2.tellg() << " bytes." << std::endl;
            return false; //size mismatch
        }

        //seek back to beginning and compare point by point
        ...
        if (!std::equal(begin1, std::istreambuf_iterator<char>(), begin2)) {
            std::cerr << "Validation failed: File contents do not match." << std::endl;
            return false;
        }
    ...
    PerformanceMetrics testS3ClientAsync(const TestConfig& config) {
        ...
        for (int i = 0; i < config.requestCount; ++i) {
            ...
            request.SetResponseStreamFactory([=]() {
                return Aws::New<DirectIOStream>("S3DirectIOStream", outputFileName);
            });
    ...
    bool downloadReferenceFile(const std::string& bucket, const std::string& key, const std::string& path, const size_t objectSize) {
        Aws::Client::ClientConfiguration clientConfig;
        clientConfig.region = "us-west-2";
        Aws::S3::S3Client s3Client(clientConfig);
        ...
        request.SetRange(ToAwsString(FormatRange(0, objectSize)));


        request.SetResponseStreamFactory([=]() { 
            return Aws::New<Aws::FStream>("ReferenceDownload", path, std::ios_base::out | std::ios_base::binary);
        });

        auto outcome = s3Client.GetObject(request);
        if (outcome.IsSuccess()) {
            std::cout << "✓ Reference file downloaded successfully to " << path << std::endl;
            return true;
        } else {
            std::cerr << "✗ Failed to download reference file: " << outcome.GetError().GetMessage() << std::endl;
            return false;
        }
    }
...
void printMetrics(const std::string& clientType, const PerformanceMetrics& metrics) {
    ...
    std::cout << "Total Bytes:           " << metrics.totalBytesTransferred << " bytes ("
              << (metrics.totalBytesTransferred * 1.0 / (1024.0 * 1024.0)) << " MB)" << std::endl;
